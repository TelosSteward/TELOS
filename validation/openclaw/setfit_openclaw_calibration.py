#!/usr/bin/env python3
"""
SetFit OpenClaw Calibration Analysis (P0: Threshold Calibration)
================================================================
Post-hoc Platt scaling on SetFit CV predictions for the OpenClaw domain.
...
Problem: Raw SetFit optimal thresholds may span a wide range across 5 folds.
...
Adapted from: validation/healthcare/setfit_calibration.py

Usage:
  python3 validation/openclaw/setfit_openclaw_calibration.py
  python3 validation/openclaw/setfit_openclaw_calibration.py results.json
  python3 validation/openclaw/setfit_openclaw_calibration.py results.json --output calibration.json
  python3 validation/openclaw/setfit_openclaw_calibration.py results.json --verbose
...
DEFAULT_RESULTS = SCRIPT_DIR / "setfit_openclaw_results.json"
...
def load_predictions(results_path: Path) -> Tuple[List[Dict], Dict]:
    ...
    if not psp:
        ...
        print("Re-run setfit_openclaw_mve.py with --verbose to generate predictions.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="SetFit OpenClaw Calibration Analysis (P0: Threshold Calibration)"
    )
    parser.add_argument(
        "results_json", nargs="?",
    ...
    print("=" * 70)
    print("SetFit OpenClaw Calibration Analysis (P0: Threshold Calibration)")
    print("=" * 70)
    ...
    print(f"  {'':20} {'Raw (t={:.4f})'.format(raw_thresh):>20} {'Calibrated (t=0.5)':>20}")
    ...
    # --- Per-tool-group calibrated performance ---
    print(f"\n--- Per-Tool-Group (Calibrated, t=0.5) ---")
    from collections import defaultdict
    group_results = defaultdict(lambda: {"tp": 0, "fn": 0, "fp": 0, "tn": 0})
    for i, p in enumerate(predictions):
        tg = p.get("tool_group", "unknown")
        pred = int(platt_scores[i] >= 0.5)
        true = p["true_label"]
        if true == 1 and pred == 1:
            group_results[tg]["tp"] += 1
        elif true == 1 and pred == 0:
            group_results[tg]["fn"] += 1
        elif true == 0 and pred == 1:
            group_results[tg]["fp"] += 1
        else:
            group_results[tg]["tn"] += 1

    for tg in sorted(group_results.keys()):
        r = group_results[tg]
        n = r["tp"] + r["fn"] + r["fp"] + r["tn"]
        det = r["tp"] / max(r["tp"] + r["fn"], 1)
        fpr_g = r["fp"] / max(r["fp"] + r["tn"], 1)
        print(f"  {tg:>15}: n={n:>3}  det={det:.1%}  fpr={fpr_g:.1%}  "
    ...
    print(f"    3. Use fixed threshold 0.50 for all tool groups")
    ...
    cal_output = {
        ...
        "per_tool_group": {
            tg: {
                ...
                "detection_rate": round(r["tp"] / max(r["tp"] + r["fn"], 1), 4),
                "fpr": round(r["fp"] / max(r["fp"] + r["tn"], 1), 4),
            }
            for tg, r in sorted(group_results.items())
        },
    ...
    output_path = (
        ...
        else results_path.parent / "setfit_openclaw_calibration.json"
    )

...
# Import from telos_core for agentic tool matching
try:
    from telos_core.semantic_specs import (
        ActionDecision,
        interpret_intent_to_tool_fidelity,
    ...
    TELOS_CORE_AVAILABLE = False
    # Fallback values if telos_core not installed
...
class TelosGovernanceGate:
    ...
    def _process_tool_call(
        ...
        zone = get_fidelity_zone(fidelity)
        intervention = get_intervention_level(fidelity, raw_sim)
        ...
        # =================================================================
        if self.use_agentic_thresholds and TELOS_CORE_AVAILABLE:
            # Use ActionDecision-based governance from telos_core
            tool_match = interpret_intent_to_tool_fidelity(
                fidelity=fidelity,
            ...
            result["action_decision"] = tool_match.decision.value
            result["confidence"] = tool_match.confidence
            result["decision_reason"] = tool_match.reason

            # EXECUTE: High-fidelity match (>= 0.85)
            if tool_match.decision == ActionDecision.EXECUTE:
                result["approved"] = True
                result["approval_source"] = "auto_agentic"
                return result

            # CLARIFY: Close match (0.70-0.84) - request confirmation
            if tool_match.decision == ActionDecision.CLARIFY:
                ...
                result["reason"] = tool_match.reason
                return result

            # SUGGEST: Vague match (0.50-0.69) - suggest alternatives
            if tool_match.decision == ActionDecision.SUGGEST:
                approval = self._request_approval(
                    action_text=action_text,
                    fidelity=fidelity,
                    zone=zone,
            ...
            if tool_match.decision == ActionDecision.ESCALATE:
                logger.warning(
            ...
            if tool_match.decision == ActionDecision.INERT:
        ...
        if intervention == InterventionLevel.HARD_BLOCK:
            if self.high_risk:
        ...
        if intervention == InterventionLevel.NONE:
            # GREEN zone - auto-approve if configured
            result["approved"] = True
            result["approval_source"] = "auto"
            return result

        if intervention == InterventionLevel.CONTEXT:
    ...
    def _update_state_after_governance(
        ...
        for blocked in blocked_calls:
            ...
            trace.append(GovernanceTraceEntry(
                ...
                intervention_reason=blocked["reason"],
                approved=False,
        ...
        state["intervention_count"] = state.get("intervention_count", 0) + len(blocked_calls)
...
def telos_governance_node(
    ...
    **kwargs,
    ...
    Usage:
        from telos_langgraph import telos_governance_node

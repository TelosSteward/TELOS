...
class ConversationManager:
    ...
    def add_assistant_message(self, content: str, timestamp: float) -> None:
        """Add assistant message to history (thread-safe)."""
        with self._lock:
            self.conversation_history.append(
    ...
    def remove_last_assistant_message(self) -> Optional[str]:
        """Remove and return last assistant message (thread-safe)."""
        with self._lock:
    ...
    def get_messages_for_api(self) -> List[Dict[str, str]]:
        """
        Get conversation history formatted for LLM API (thread-safe).
    ...
    async def async_add_user_message(self, content: str, timestamp: float) -> None:
        """Add user message to history (async thread-safe)."""
    ...
    async def async_add_assistant_message(self, content: str, timestamp: float) -> None:
        """Add assistant message to history (async thread-safe)."""
        if self._async_lock is None:
            self._async_lock = asyncio.Lock()

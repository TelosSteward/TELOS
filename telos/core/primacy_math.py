...
def calculate_fidelity(embedding: np.ndarray, pa: np.ndarray) -> float:
    """Calculate fidelity (cosine similarity) between embedding and PA.

    Wrapper around calculate_cosine_similarity for semantic clarity.
    Handles zero vectors gracefully by returning 0.0 (maximum deviation).
    ...
    """
    return calculate_cosine_similarity(embedding, pa)


def normalize_vector(vector: np.ndarray, epsilon: float = 1e-10) -> np.ndarray:
    """Safely normalize vector with zero-vector handling.

    Standard normalization utility to avoid code duplication across modules.

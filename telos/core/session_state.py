...
class SessionStateManager:
    ...
    MAX_HISTORY_SIZE = 1000  # Keep last 1000 turns
    # Number of important snapshots to always preserve (triggers, interventions)
    PRESERVE_IMPORTANT = 100

    def __init__(self, web_session_manager: Optional[Any] = None, max_history_size: int = None):
        """
        Initialize session state manager with bounded history.

        Args:
            web_session_manager: WebSessionManager for UI integration (optional)
            max_history_size: Maximum number of snapshots to retain (default: 1000)
        """
        self.web_manager = web_session_manager
        self._snapshots: List[TurnSnapshot] = []
        self._important_turns: set = set()  # Track important turns to preserve
        self.max_history_size = max_history_size or self.MAX_HISTORY_SIZE
        self._session_metadata: Dict[str, Any] = {
            'session_id': f"session_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            'started_at': datetime.now().isoformat(),
            'total_turns': 0,
            'pruned_turns': 0  # Track how many turns have been pruned
        }
    ...
    def save_turn_snapshot(
        ...
        # Mark important turns (low fidelity or interventions)
        if snapshot.telic_fidelity < 0.7 or not snapshot.basin_membership:
            self._important_turns.add(turn_number)

        # Prune history if exceeded max size
        self._prune_history_if_needed()

        self._session_metadata['total_turns'] = len(self._snapshots) + self._session_metadata.get('pruned_turns', 0)
    ...
    def _prune_history_if_needed(self) -> None:
        ...
        Preserves important turns (triggers/interventions) when possible.
        """
        if len(self._snapshots) <= self.max_history_size:
            return

        # Identify snapshots to keep
        num_to_prune = len(self._snapshots) - self.max_history_size

        # Separate important and regular snapshots
        important_snapshots = []
        regular_snapshots = []

        for i, snapshot in enumerate(self._snapshots[:num_to_prune]):
            if snapshot.turn_number in self._important_turns:
                important_snapshots.append(snapshot)
            else:
                regular_snapshots.append(snapshot)

        # Prune regular snapshots first
        snapshots_to_keep = self._snapshots[num_to_prune:]

        # Add back some important snapshots if we have room
        if len(important_snapshots) > 0:
            # Keep the most recent important ones
            num_important_to_keep = min(
                len(important_snapshots),
                self.PRESERVE_IMPORTANT
            )
            snapshots_to_keep = (
                important_snapshots[-num_important_to_keep:] +
                snapshots_to_keep
            )

        # Update pruned count
        self._session_metadata['pruned_turns'] += (
            len(self._snapshots) - len(snapshots_to_keep)
        )

        self._snapshots = snapshots_to_keep

...
def check_dual_pa_fidelity(
    ...
    """
    # Validate response embedding dimension
    expected_dim = getattr(embedding_provider, 'dimension', None)
    if expected_dim is None:
        expected_dim = getattr(embedding_provider, 'expected_dim', None)

    if expected_dim and response_embedding.shape[0] != expected_dim:
        raise ValueError(
            f"Response embedding dimension mismatch: got {response_embedding.shape[0]}, "
    ...
    if user_p_vec.shape[0] != response_embedding.shape[0]:
        raise ValueError(
            f"User purpose vector dimension mismatch: got {user_p_vec.shape[0]}, "
            f"response has {response_embedding.shape[0]}"
        )
    if user_s_vec.shape[0] != response_embedding.shape[0]:
        raise ValueError(
            f"User scope vector dimension mismatch: got {user_s_vec.shape[0]}, "
            f"response has {response_embedding.shape[0]}"
        )
    ...
    # Validate AI PA dimension consistency
    if ai_p_vec.shape[0] != response_embedding.shape[0]:
        raise ValueError(
            f"AI purpose vector dimension mismatch: got {ai_p_vec.shape[0]}, "
            f"response has {response_embedding.shape[0]}"
        )
    if ai_s_vec.shape[0] != response_embedding.shape[0]:
        raise ValueError(
            f"AI scope vector dimension mismatch: got {ai_s_vec.shape[0]}, "
            f"response has {response_embedding.shape[0]}"
        )

...
Generate a natural user message that continues the discussion (just the message, no labels):"""
    ...
    def _calculate_salience(
        ...
        # Zero vector handling: treat as maximum deviation (similarity = 0)
        # Using epsilon threshold for numerical stability
        if norm_context < 1e-10 or norm_attractor < 1e-10:
            similarity = 0.0
        else:
            similarity = float(np.dot(context_emb, attractor_center) /
                             (norm_context * norm_attractor))

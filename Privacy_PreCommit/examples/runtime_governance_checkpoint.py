...
try:
    ...
    print("Make sure all Runtime Governance files are in the same directory.")
    sys.exit(1)
...
def load_session():
    ...
    if not session_file.exists():
        ...
        print("Run: python3 runtime_governance_start.py")
        sys.exit(1)
...
def analyze_turn(user_message, assistant_response, turn_number, session_info):
    ...
    checkpoint_file = f'.runtime_governance_checkpoint_{turn_number}.json'
    ...
    if turn_number > 1:
        ...
        for i in range(1, turn_number):
            cp_file = Path(f'.runtime_governance_checkpoint_{i}.json')
            if cp_file.exists():

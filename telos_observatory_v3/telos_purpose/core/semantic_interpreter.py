...
def interpret(fidelity: float, purpose: str) -> SemanticSpec:
    ...
    if strength < 0.45:
        ...
        # User barely drifted - lightest possible touch
        return SemanticSpec(
        ...
        # Gentle guidance, conversational
        return SemanticSpec(
        ...
        # Clear redirect, anchoring statement
        return SemanticSpec(
...
# =============================================================================
# This is the "corpus" that gets "retrieved" based on fidelity value.
# Multiple variations per strength band create natural variety.
# No external database - just string injection during processing.
# =============================================================================
...
def get_exemplar(strength: float, purpose: str = "{purpose}") -> Optional[str]:
    ...
    This is the "retrieval" step of internal RAG - fidelity value is the query,
    corpus is the exemplar dictionary, response is the matched exemplar.

    Uses deterministic selection based on strength value to ensure reproducibility
    while still providing variety across different fidelity levels within a band.
    ...
    # Select corpus band based on strength
    if strength < 0.45:
        band = "minimal"
    elif strength < 0.60:
        band = "light"
    ...
    # Deterministic but varied selection within band
    # Use strength value to pick which exemplar (creates variety without randomness)
    # Multiply by 1000 to get variation from decimal places
    index = int((strength * 1000) % len(exemplars))

    # Interpolate purpose into the selected exemplar
    return exemplars[index].format(purpose=purpose)


def compute_behavioral_fidelity(ai_response: str, user_fidelity: float, embedding_provider=None) -> float:
    """
    Compute AI Behavioral Fidelity during interventions.

    Instead of measuring AI response vs User PA (which is misleading during interventions),
    this measures how well the AI response matches expected intervention behavior by
    comparing against the EXEMPLAR_CORPUS.

    This implements the "Behavioral PA" concept - measuring AI against its expected
    behavior pattern rather than semantic content similarity.

    Args:
        ai_response: The AI's intervention response text
        user_fidelity: User fidelity that triggered intervention (determines expected band)
        embedding_provider: Embedding provider for similarity calculation.
                           If None, uses cached MPNet provider.

    Returns:
        Behavioral fidelity score (0.0 to 1.0) in display-normalized range.
    ...
    # Determine which band the intervention should have used based on user fidelity
    ...
    # Get all exemplars for this band (with generic placeholder for purpose)
    exemplars = EXEMPLAR_CORPUS[band]
    ...
    for exemplar in exemplars:
        # Use generic placeholder to focus on behavioral pattern, not purpose content
...
def get_behavioral_fidelity_band(user_fidelity: float) -> str:
    ...
    Useful for logging and debugging behavioral fidelity calculations.

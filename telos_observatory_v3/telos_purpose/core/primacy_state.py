...
class PrimacyStateCalculator:
    ...
    def compute_primacy_state(
        ...
        DUAL formula: PS = ρ_PA · (2·F_user·F_AI)/(F_user + F_AI)
        TRIFECTA formula: PS = ρ_PA · 3/(1/F_user + 1/F_AI + 1/F_steward)
        ...
        if self.mode == 'trifecta':
            ...
            if steward_emb is not None:
                ...
                # Steward gatekeeper evaluation
                try:
                    from config.steward_pa import steward_evaluation
                    eval_result = steward_evaluation(F_steward)
                    steward_ruling = eval_result['ruling']
                except ImportError:
                    # Fallback thresholds calibrated for raw SentenceTransformer scores
                    # (typical range: -0.1 to 0.5 for cosine similarity)
                    if F_steward >= 0.25:

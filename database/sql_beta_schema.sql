...
-- Stores governance metrics for beta sessions (NOT conversation content)
-- Privacy: Only mathematical measurements are stored, never text content

-- ============================================================================
-- beta_sessions: Session-level metadata
-- ============================================================================

CREATE TABLE beta_sessions (
    session_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),

    -- PA Configuration (stored as JSONB)
...
-- ============================================================================
-- beta_turns: Turn-level governance data
-- ============================================================================
...
CREATE TABLE beta_turns (
    ...
    -- User Input (stored for post-session review only)
    user_message TEXT NOT NULL,

    -- AB Testing: Which system was served
    system_served TEXT CHECK (system_served IN ('telos', 'native')),

    -- Responses (stored for post-session review only)
    telos_response TEXT NOT NULL,
    native_response TEXT NOT NULL,
    response_delivered TEXT NOT NULL,

    -- Governance Metrics (PRIMARY DATA)
    user_fidelity FLOAT,
    ai_fidelity FLOAT,
    primacy_state FLOAT,
    ...
    -- Steward Interpretation (LLM-generated explanation)
...
-- Indexes for turn retrieval and analysis
...
-- IMPORTANT: This schema stores conversation content (user_message, responses)
-- for post-session review purposes. However:
--
-- 1. Sessions are ephemeral (one sitting, browser-local)
-- 2. Content is ONLY stored during active session for review
-- 3. Users are informed via beta_onboarding.py that:
--    - Governance metrics (fidelity, distance, etc.) are collected
--    - Conversation content is used for review but not analyzed
-- 4. Future: Consider TTL policy to auto-delete old sessions
-- 5. Research exports: Only metrics tables, never full conversation content

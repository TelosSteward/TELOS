...
def _resolve_llm_key(provider_name: str) -> str:
    ...
    Never forwards caller-supplied keys to upstream providers.
    """
    env_map = {
    ...
    if not key:
        raise HTTPException(
            ...
            detail={
                "error": {
                    "message": "LLM provider not configured on this server",
...
async def chat_completions(
    ...
    try:
        ...
        # Memory clearing: explicitly delete request/response content references
        # to minimize residual plaintext in Python's garbage-collected heap.
        # This is best-effort — Python GC may retain copies.
        try:
            del request

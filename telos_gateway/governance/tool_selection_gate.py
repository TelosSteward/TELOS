            ...
            "tool_rankings": [
                {
                    ...
                    "description": score.tool_description[:80] + "..." if len(score.tool_description) > 80 else score.tool_description,
                }
...
class ToolSelectionGate:
    ...
    def _generate_reasoning(
        ...
        if selected.display_percentage >= 90:
            return (
                f"Selected '{selected.tool_name}' with {selected.display_percentage}% semantic alignment. "
                f"The request strongly matches this tool's purpose: {selected.tool_description[:60]}..."
            )
        ...
        # Moderate confidence
            ...
            reasoning = (
                f"Selected '{selected.tool_name}' with {selected.display_percentage}% semantic alignment. "
                f"Good match for the request."
            )
            # Mention runner-up if close
            if len(all_scores) > 1 and all_scores[1].display_percentage >= 75:
                reasoning += f" Alternative: '{all_scores[1].tool_name}' ({all_scores[1].display_percentage}%)."
            return reasoning
        ...
        # Low confidence - warn about potential mismatch
            ...
            return (
                f"Selected '{selected.tool_name}' with {selected.display_percentage}% semantic alignment. "
                f"Moderate match - request may be ambiguous. "
        ...
        # Very low - likely wrong tool
            ...
            return (
                f"Warning: Best match is '{selected.tool_name}' with only {selected.display_percentage}% alignment. "
                f"The request may not match any available tool well. Consider clarifying the request."
            )
...
SQL_AGENT_TOOLS = [
    ToolDefinition(
        name="sql_db_query",
        description="Execute a SQL query against the database and return results. Use for retrieving data, running SELECT statements, and getting query results.",
    ),
    ToolDefinition(
        name="sql_db_schema",
        description="Get the schema and sample rows for specified SQL tables. Use to understand table structure, column types, and see example data.",
    ),
    ToolDefinition(
    ...
    ToolDefinition(
        ...
        description="Check if a SQL query is correct before executing. Use to validate syntax and catch potential errors.",
    ),
]
...
RESEARCH_AGENT_TOOLS = [
    ToolDefinition(
        name="web_search",
        description="Search the web for information on any topic. Use for finding current information, news, and general research.",
    ),
    ToolDefinition(
        name="wikipedia",
        description="Look up information on Wikipedia. Use for factual information, historical data, and encyclopedic knowledge.",
    ),
    ToolDefinition(
        name="calculator",
        description="Perform mathematical calculations. Use for arithmetic, statistics, and numerical analysis.",
    ),
    ToolDefinition(
        name="summarize",
        description="Summarize long text into key points. Use after gathering information to create concise summaries.",
    ),
]
...
CUSTOMER_SERVICE_TOOLS = [
    ToolDefinition(
        name="lookup_order",
        description="Look up customer order information by order ID or customer ID. Use to find order status, details, and history.",
    ),
    ToolDefinition(
        name="process_refund",
        description="Process a refund for a customer order. Use when customer requests refund and order is eligible.",
    ),
    ToolDefinition(
        name="escalate_to_human",
        description="Escalate the conversation to a human agent. Use for complex issues, complaints, or sensitive matters.",
    ),
    ToolDefinition(
        name="search_faq",
        description="Search the FAQ knowledge base for answers. Use for common questions about products, policies, and procedures.",
    ),
]

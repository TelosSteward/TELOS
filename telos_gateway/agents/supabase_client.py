...
class SupabaseConfig:
    ...
    # Direct PostgreSQL connection string (for raw SQL)
...
class SupabaseClient:
    ...
    Supports two modes:
    1. Direct PostgreSQL (psycopg2) - for raw SQL queries
    2. Supabase REST API - for ORM-style queries
    ...
    def connect(self) -> bool:
        ...
        """
        # Try direct PostgreSQL connection first (more powerful for SQL agent)
        if PSYCOPG2_AVAILABLE and self.config.postgres_url:
            try:
        ...
        # Fall back to Supabase REST client
        if SUPABASE_AVAILABLE and self.config.url and self.config.key:
            try:
                ...
                logger.error(f"Supabase REST connection failed: {e}")

        logger.error("No valid Supabase connection method available")
        return False
    ...
    def execute_query(
        ...
        # Execute via PostgreSQL if available
        if self._pg_conn:
            return self._execute_pg(query, params)
        else:
            return {
                "success": False,
                "error": "Direct SQL requires PostgreSQL connection. REST API doesn't support raw SQL.",
                "data": [],
                "columns": [],
                "row_count": 0,
            }
    ...
    def get_sample_rows(
        ...
        # Note: table/schema names can't be parameterized in psycopg2

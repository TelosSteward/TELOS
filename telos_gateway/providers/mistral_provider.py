...
class MistralProvider(LLMProvider):
    ...
    async def chat_completion(
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail={"error": {"message": "Upstream LLM provider error", "type": "provider_error", "code": 502}},
            )
        except httpx.RequestError:
            logger.error("Mistral API connection error", exc_info=True)
            raise HTTPException(
                status_code=502,
                detail={"error": {"message": "Could not reach LLM provider", "type": "provider_error", "code": 502}},
            )
    ...
    async def chat_completion_stream(
        ...
        try:
            ...
            logger.error(f"Mistral streaming error: {e.response.status_code}")
            yield f'data: {json.dumps({"error": {"message": "Upstream LLM provider error", "type": "provider_error"}})}\n\n'
        except httpx.RequestError:
            logger.error("Mistral streaming connection error", exc_info=True)
            yield f'data: {json.dumps({"error": {"message": "Could not reach LLM provider", "type": "provider_error"}})}\n\n'

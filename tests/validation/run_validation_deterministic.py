#!/usr/bin/env python3
"""
Forensic validation using DETERMINISTIC EMBEDDINGS for comparison.
Uses hash-based embeddings instead of semantic ones.
"""
import json
import sys
import os
...
def main():
    """Run validation using deterministic embeddings."""

    print("🔬 TELOS VALIDATION WITH DETERMINISTIC EMBEDDINGS")
    print("="*80)
    print("⚠️  Using HASH-BASED embeddings (no semantic meaning) for comparison")
    print()

    # Initialize validator with custom embedder
    validator = ForensicValidator()

    # Override embedder with deterministic version
    print("📦 Loading Deterministic Embeddings (hash-based, no semantic meaning)...")
    validator.embedder = EmbeddingProvider(deterministic=True)
    print(f"✅ Deterministic embeddings loaded")
    ...
    validator.report = {
        "metadata": {
            ...
            "embedding_provider": "SentenceTransformer (local)"
        },
        "pa_establishment": {

...
class ConversationDisplay:
    ...
    def _render_main_chat(self):
        ...
        demo_mode = st.session_state.get('telos_demo_mode', False)
        beta_mode = st.session_state.active_tab == "BETA"
        ...
        if beta_mode:
            ...
            st.sidebar.write(f"beta_intro_complete: {beta_intro_complete}")
            st.sidebar.write(f"turns count: {len(all_turns)}")
            st.sidebar.write(f"beta_intro_slide: {st.session_state.get('beta_intro_slide', 'not set')}")
            if len(all_turns) > 0:
                st.sidebar.write(f"Latest turn has response: {bool(all_turns[-1].get('response'))}")
                st.sidebar.write(f"Latest turn is_streaming: {all_turns[-1].get('is_streaming', False)}")
        ...
        if len(all_turns) == 0:
            # Blank session - check Demo Mode ONLY (Beta intro already handled above)

...
class AsyncStewardProcessor:
    ...
    async def process_batch(self, items: list):
        ...
        tasks = []
        for item in items:
            # Ensure we await async process_item calls
            if hasattr(self, 'process_item'):
                tasks.append(self.process_item(item))
            else:
                # Fallback for items that need processing
                tasks.append(self._process_single_item(item))
    ...
    async def _process_single_item(self, item):
        ...
        # In real implementation, this would call actual processing logic
        await asyncio.sleep(0.01)  # Simulate async work
        return f"Processed: {item}"

    async def async_intervention_check(self, session_id: str):
        """
        ...
        logger.info(f"Checked intervention for session {session_id}")
        return False

    async def check_all_sessions(self, active_sessions: list):
        """
        Check all active sessions for needed interventions.

        FIXED: Properly await all async intervention checks.

        Args:
            active_sessions: List of session IDs to check
        """
        tasks = []
        for session_id in active_sessions:
            # FIXED: Create tasks for concurrent checking
            tasks.append(self.async_intervention_check(session_id))

        # FIXED: Properly await all tasks
        results = await asyncio.gather(*tasks, return_exceptions=True)

        # Log results
        for session_id, result in zip(active_sessions, results):
            if isinstance(result, Exception):
                logger.error(f"Session {session_id} check failed: {result}")
            elif result:
                logger.info(f"Session {session_id} needs intervention")

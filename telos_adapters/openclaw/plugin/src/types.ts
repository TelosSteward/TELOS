...
export type GovernanceDecision =
  | "execute"   // High confidence, proceed
  | "clarify"   // Verify intent first
  | "suggest"   // Offer alternatives
  | "inert"     // Acknowledge limitation
...
export interface IPCGateTransitionMessage {
  ...
  request_id: string;
  args?: { gate_file_path?: string };
}

export type IPCMessage =
  | IPCScoreMessage
  | IPCHealthMessage
  | IPCResetChainMessage
  | IPCShutdownMessage
  | IPCAcknowledgeDriftMessage
  | IPCGetDriftStatusMessage
  | IPCGateTransitionMessage;
...
export type IPCResponse =
  ...
  | IPCErrorResponse
  | IPCAckResponse
  | IPCEscalationPendingResponse
  | IPCDriftStatusResponse;

 ...
 */
function buildActionText(toolName, params) {
    const parts = [];
    ...
    return parts.join(" ") || `${toolName} (no description)`;
}
/**
 * Format a block message for the user.
 */
function resolveTelosAgentId(toolName, params, ctx) {
    const fromInput = (() => {
        const input = (params || {});
    ...
    if (fromInput)
        return fromInput;
    const contextAgentId = typeof ctx?.agentId === "string"
        ? ctx.agentId.trim()
        : "";
...
function formatBlockMessage(verdict, toolName) {
    const lines = [
...
async function onGatewayStart() {
    ...
    bridge = new bridge_1.TelosBridge({
...
async function onSessionEnd() {
    if (sessionStats.scored > 0) {
        ...
        log(`Session summary: ${sessionStats.scored} scored, ` +
...
async function onBeforeToolCall(event, ctx) {
    ...
    if (!bridge) {
        ...
        if (config?.failPolicy === "closed") {
            log(`BLOCKED (no daemon): ${toolName}`);
            return null; // Block
        }
        return event; // Allow (fail-open)
    }
    const actionText = buildActionText(toolName, params);
    try {
        const telosAgentId = resolveTelosAgentId(toolName, params, ctx);
        const scoringArgs = {
            ...params,
            __session_key: ctx?.sessionKey || "",
        };
        if (telosAgentId) {
            scoringArgs.agent_id = telosAgentId;
            scoringArgs.TELOS_AGENT_ID = telosAgentId;
        }
        const verdict = await bridge.scoreAction(toolName, actionText, scoringArgs);
        // Update stats
        sessionStats.scored++;
        ...
        if (!verdict.allowed) {
            ...
            if (verdict.decision === "escalate") {
                ...
                const overrideInfo = verdict.override_receipt
                    ? ` (override receipt: ${verdict.override_receipt?.payload_hash?.substring(0, 16)}...)`
                    : "";
                log(`[TELOS] Escalation ${verdict.human_required ? "denied/timed out" : "blocked"}: ` +
                    `${toolName} (${verdict.risk_tier} risk)${overrideInfo}`);
            }
            log(formatBlockMessage(verdict, toolName));
            return null; // Block the tool call
        }
        if (verdict.decision === "escalate") {
            sessionStats.escalated++;
            // Override was approved — log the receipt hash
            const receipt = verdict.override_receipt;
            if (receipt) {
                log(`[TELOS] Override approved: ${toolName} ` +
        ...
        if (config.verbose) {
            log(`[${verdict.decision.toUpperCase()}] ${toolName}: ` +
                `fidelity=${verdict.fidelity.toFixed(3)} ` +
...
async function onMessageSending(event, ctx) {
    if (!bridge?.isConnected)
        return event;
    try {
        const channel = event.params?.channel || event.channel || "unknown";
        const content = event.params?.content || event.content || "";
        const verdict = await bridge.scoreAction("SendMessage", `Send message to ${channel}: ${content}`, { channel, message: content });
        if (!verdict.allowed) {
            log(`BLOCKED message to ${channel}: ${verdict.explanation}`);
            return null;
        }
        return event;
    }
...
exports.default = {
    ...
    register(api) {
        ...
        api.on("message_sending", async (event, ctx) => {
            const result = await onMessageSending(event, ctx);
            if (result === null) {
